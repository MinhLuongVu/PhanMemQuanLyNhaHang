...
namespace QuanLyNhaHang.ViewModel
{
    public class MainViewModel : BaseViewModel 
    {
        ...
        private ObservableCollection<Model.TRANGTHAI> _TRANGTHAI;
        public ObservableCollection<Model.TRANGTHAI> TRANGTHAI { get => _TRANGTHAI; set { _TRANGTHAI = value; OnPropertyChanged(); } }
        ...
        public void XuLyDanhSachBan()
        {
            ...
            }, (p) =>
            {
                ...
                {
                    if (!Regex.IsMatch(TenBan, @"^.{5,}$"))
                    {
                        MessageBox.Show("Tên chức vụ phải trên 5 kí tự!", "Thông báo");
                    }
                    else
                    {
                        try
                        {
                            var MaBan = DataProvider.Ins.DB.DANHSACHBANs.Where(x => x.TenBan == TenBan).SingleOrDefault();
                            if(MaBan!=null)
                            {
                                MessageBox.Show("Tên bàn trùng!", "Thông báo!");
                            }
                            else
                            {
            ...
            }, (p) =>
            {
                ...
                else
                {
                    if (!Regex.IsMatch(TenBan, @"^.{5,}$"))
                    {
                        MessageBox.Show("Tên bàn phải trên 5 kí tự!", "Thông báo");
                    }
                    else
                    {
            ...
            }, (p) =>
            {
                try
                {
                    ...
                    int MaBan = SelectedItemDanhsachban1.DANHSACHBAN.MaBan;
                    var GoiMon = DataProvider.Ins.DB.GOIMONs.Where(x => x.MaBan == MaBan).SingleOrDefault();
                    if (GoiMon != null)
                    {
                        MessageBox.Show("Bạn không thể xóa bàn này khi khách đang ngồi!", "Thông báo!");
                    }
                    else
                    {
                        DANHSACHBAN= new ObservableCollection<DANHSACHBAN>(DataProvider.Ins.DB.DANHSACHBANs.Where(x => x.MaBan == MaBan));
                        DataProvider.Ins.DB.DANHSACHBANs.RemoveRange(DANHSACHBAN);
                        DataProvider.Ins.DB.SaveChanges();
                        LoadDanhSachBan();
                        SelectedItemDanhsachban1 = null;
                        TenBan = null;
                        MessageBox.Show("Bạn đã bàn thành công!", "Thông báo!");
                    }
            ...
            }, (p) =>
            {
                ...
                else
                {
                    ...
                    else
                    {
                        if (Tien >= TongTien)
                        {
                            TRANGTHAI = new ObservableCollection<TRANGTHAI>(DataProvider.Ins.DB.TRANGTHAIs.Where(X => X.MaBan == SelectedItem.MaBan));
                            ...
                            fd.Blocks.Add(new Paragraph(new Run(
                               a+":\t"+ String.Format("{0:n0}", TongTien)+" VNĐ"))
                                    );
                            fd.Blocks.Add(new Paragraph(new Run(
                            ...
                            DataProvider.Ins.DB.TRANGTHAIs.RemoveRange(TRANGTHAI);
                            DataProvider.Ins.DB.SaveChanges();
                            GOIMON = null;
            ...
            }, (p) =>
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (!Regex.IsMatch(HoTenNV, @"^.{8,}$"))
                            {
                                MessageBox.Show("Họ Tên NV phải trên 8 kí tự!", "Thông báo");
                            }
                            else
                            {
                                if (HeThong.ToString() == "Android" || HeThong.ToString() == "PC" || HeThong.ToString() == "android" || HeThong.ToString() == "pc")
                                {
                                    try
                                    {
                                        var MaNV = DataProvider.Ins.DB.NHANVIENs.Where(x => x.TenDN == TenDN).SingleOrDefault();
                                        if (MaNV == null)
                                        {
                                        ...
                                        {
                                            MessageBox.Show("Tên đăng nhập đã trùng!", "Thông báo");
                                        }
                                    }
                                    catch(Exception)
                                    {
        ...
        public void XuLyDanhSachLoaiMon()
        {
            ...
            }, (p) =>
            {
                ...
                {
                    if (!Regex.IsMatch(TenLoaiMon, @"^.{5,}$"))
                    {
                        MessageBox.Show("Tên loại món phải trên 5 kí tự!", "Thông báo");
                    }
                    else
                    {
                        try
                        {
                            var MaLoaiMon1 = DataProvider.Ins.DB.LOAIMONs.Where(x => x.TenLoaiMon == TenLoaiMon).SingleOrDefault();
                            if(MaLoaiMon1!=null)
                            {
                                MessageBox.Show("Tên loại món trùng!", "Thông báo!");
                            }
                            else
                            {
            ...
            }, (p) =>
            {
                try
                {
                    int MaLoaiMon = SelectedItemLoaiMonAn1.LOAIMON.MaLoaiMon;
                    var LoaiMon = DataProvider.Ins.DB.MONANs.Where(x => x.MaLoaiMon== MaLoaiMon).SingleOrDefault();
                    if (LoaiMon!= null)
                    {
                        MessageBox.Show("Bạn không thể xóa loại món này khi món ăn có loại món trên!", "Thông báo!");
                    }
                    else
                    {
                        LOAIMON = new ObservableCollection<LOAIMON>(DataProvider.Ins.DB.LOAIMONs.Where(x => x.MaLoaiMon == MaLoaiMon));
                        DataProvider.Ins.DB.LOAIMONs.RemoveRange(LOAIMON);
                        DataProvider.Ins.DB.SaveChanges();
                        LoadLoaiMonAn();
                        SelectedItemLoaiMonAn1 = null;
                        TenLoaiMon = null;
                        MessageBox.Show("Bạn đã xóa loại món ăn thành công!", "Thông báo!");
                    }
        ...
        void LoadDanhSachMonAn1()
        {
            ...
            foreach (var item in aMonAn1)
            {
                ...
                TenMon = item.TenMon;
                MaMon =string.Format(item.MaMon.ToString());
                MonAn monan = new MonAn();
                monan.aDonGia = DonGia;
                monan.aHinhAnh = HinhAnh;
                monan.aTenMon = TenMon;
                monan.aMaMon =int.Parse(MaMon);
        ...
        private void XuLyMonAn()
        {
            ...
            }, (p) =>
            {
                ...
                {
                    if(!Regex.IsMatch(TenMon, @"^.{5,}$"))
                    {
                        MessageBox.Show("Tên món phải từ 8 kí tự trở lên!", "Thông báo!");
                    }
                    else
                    {
                        if (!Regex.IsMatch(DonGia.ToString(), @"^.{5,}$"))
                        {
                            MessageBox.Show("Đơn giá món phải trên 10.000 VNĐ!", "Thông báo!");
                        }
                        else
                        {
                            if(HinhAnh == null)
                            {
                                MessageBox.Show("Vui lòng tải ảnh lên", "Thông báo!");
                            }
                            else
                            {
                                try
            ...
            }, (p) =>
            {
                ...
                {
                    var MaMon = DataProvider.Ins.DB.MONANs.Where(x => x.MaMon == SelectedItemMonAn.aMaMon).SingleOrDefault();
                    MaMon.TenMon = TenMon;
                    MaMon.DonGia = DonGia;
                    DataProvider.Ins.DB.SaveChanges();
                    MessageBox.Show("Bạn đã cập nhật thông tin món ăn thành công!", "Thông báo!");
                    LoadDanhSachMonAn1();
                    LoadDanhSachMonAn();
            ...
            }, (p) =>
            {
                try
                {
                    int MaMon = SelectedItemMonAn.aMaMon;
                    var MonAn = DataProvider.Ins.DB.GOIMONs.Where(x => x.MaMon == MaMon).SingleOrDefault();
                    if (MonAn != null)
                    {
                        MessageBox.Show("Bạn không thể xóa món ăn khi món ăn đang được order!", "Thông báo!");
                    }
                    else
                    {
                        MONAN = new ObservableCollection<MONAN>(DataProvider.Ins.DB.MONANs.Where(x => x.MaMon == MaMon));
                        DataProvider.Ins.DB.MONANs.RemoveRange(MONAN);
                        DataProvider.Ins.DB.SaveChanges();
                        LoadDanhSachMonAn1();
                        LoadDanhSachMonAn();
                        TenMon = null;
                        ...
                        AddHinhAnh = null;
                        SelectedItemLoaiMonAn = null;
                        MessageBox.Show("Bạn đã xóa thành công món ăn!", "Thông báo!");
                    }
        ...
        public void XuLyDanhSachChucVu()
        {
            ...
            }, (p) =>
            {
                ...
                else
                {
                    ...
                    else
                    {
                        try
                        {
                            var MaCV = DataProvider.Ins.DB.CHUCVUs.Where(x => x.ChucVu1 == TenChucVu).SingleOrDefault();
                            if (MaCV == null)
                            {
                            ...
                            {
                                MessageBox.Show("Tên chức vụ đã trùng!", "Thông báo!");
                            }
                        }
                        catch (Exception)
                        {
            ...
            }, (p) =>
            {
                ...
                try
                {
                    ...
                    int MaChucVu = SelectedItemCHUCVU1.CHUCVU.MaChucVu;
                    var NhanVien = DataProvider.Ins.DB.NHANVIENs.Where(x => x.MaChucVu == MaChucVu).SingleOrDefault();
                    if(NhanVien!=null)
                    {
                        MessageBox.Show("Bạn không thể xóa chức vụ này khi nhân viên đang có chức vụ trên!", "Thông báo!");
                    }
                    else
                    {
                        CHUCVU = new ObservableCollection<CHUCVU>(DataProvider.Ins.DB.CHUCVUs.Where(x => x.MaChucVu == MaChucVu));
                        DataProvider.Ins.DB.CHUCVUs.RemoveRange(CHUCVU);
                        DataProvider.Ins.DB.SaveChanges();
                        LoadChucVu();
                        SelectedItemCHUCVU1 = null;
                        TenChucVu = null;
                        MessageBox.Show("Bạn đã xóa chức vụ thành công!", "Thông báo!");
                    }
        ...
        public void TimKiemThongKeDoanhThu()
        {
            ...
            {   
                DateTime NgayKetThuc = DateTime.Parse(NgayKetThucThongKeDoanhThu.ToString());
                DateTime NgayBatDau = DateTime.Parse((NgayBatDauThongKeDoanhThu.ToString()));
                String TienDoanhThu = (String.Format("{0:n0}", TongTienDoanhThu));
                FlowDocument fd = new FlowDocument();
                PrintDialog pd = new PrintDialog();
                fd.PagePadding = new Thickness(50);
                fd.ColumnGap = 0;
        ...
        public string GetMD5(string MD5)
        {
            ...
            var bytes = System.Text.Encoding.UTF8.GetBytes(MD5);
